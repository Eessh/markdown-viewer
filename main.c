...
int main(int argc, char** argv) {
  ...
  char* markdown_file_path = (char*)malloc(sizeof(char)*(markdown_file_path_length+1));
  strcpy_s(markdown_file_path, sizeof(char)*(markdown_file_path_length+1), argv[1]);
  ...
  int ret_value = md_parse(details->content, sizeof(char)*details->length, parser, NULL);
...
file_details* read_file(const char* file_path) {
  ...
  if (!buffer) {
    ...
    printf("Unable to allocate memory for reading characters in file: %s", file_path);
    fclose(fp);
    return NULL;
  }
