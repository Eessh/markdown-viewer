...
{
  // creating window
  SDL_WindowFlags prompt_window_flags =
    SDL_WINDOW_ALLOW_HIGHDPI | SDL_WINDOW_MOUSE_FOCUS | SDL_WINDOW_SHOWN;
  SDL_Window* prompt_window =
    SDL_CreateWindow("Markdown Viewer - Open markdown file",
  ...
  if(!prompt_window)
  {
    SDL_LogCritical(SDL_LOG_CATEGORY_ERROR,
                    "Unable to create window for select file prompt: %s",
                    SDL_GetError());
    return (prompt_result){
      .ok = false,
      .data = {.error_message =
                 "Unable to create window for select file prompt."}};
  }
  ...
  SDL_FillRect(prompt_window_surface,
               &(SDL_Rect){.x = 230, .y = 400, .w = 180, .h = 30},
               SDL_MapRGBA(prompt_window_surface->format, 50, 50, 50, 255));
  ...
  SDL_BlitSurface(text_surface,
                  NULL,
                  prompt_window_surface,
                  &(SDL_Rect){.x = 238, .y = 400, .w = 200, .h = 20});
      ...
      else if(event.type == SDL_MOUSEBUTTONUP)
      {
        if(event.button.button == SDL_BUTTON_LEFT)
        {
          ...
          SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                      ...
                      mouse_ycoord);
#endif
          if(230 <= mouse_xcoord && mouse_xcoord <= 410 &&
             400 <= mouse_ycoord && mouse_ycoord <= 430)
          {
            const char* file_selection_filters[1] = {"*.md"};
            markdown_file_path = tinyfd_openFileDialog("Select markdown file",
                                                       "",
                                                       1,
                                                       file_selection_filters,
                                                       "Markdown",
                                                       0);
...
// cancelled
#ifdef DEBUG
              SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                          "Cancelled select markdown file dialog.");
#endif
            }
          }
        }
  ...
  return (prompt_result){.ok = true, .data = {.file_path = markdown_file_path}};
}