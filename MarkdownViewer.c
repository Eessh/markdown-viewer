...
DWORD WINAPI threaded_parsing(LPVOID lp_param) {
  ...
  int parser_return_value = md_parse(params->text, params->size, params->parser, params->userdata);
  #ifdef DEBUG
  SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Parser return value: %d", parser_return_value);
  #endif
...
int main(int argc, char** argv) {
  ...
  if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_EVENTS) != 0) {
    SDL_LogCritical(SDL_LOG_CATEGORY_ERROR, "Unable to initialize video: %s", SDL_GetError());
    return 1;
  }
  ...
  if (TTF_Init() != 0) {
    SDL_LogCritical(SDL_LOG_CATEGORY_ERROR, "Unable to initialize ttf extension: %s", TTF_GetError());
    SDL_Quit();
    return 1;
  }
  ...
  if (!segoeui_font) {
    SDL_LogCritical(SDL_LOG_CATEGORY_ERROR, "Unable to load font: %s", segoeui_font_file_path);
    TTF_Quit();
    SDL_Quit();
    return 1;
  }
  SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Loaded font: %s", segoeui_font_file_path);
  ...
  if (argc < 2) {
    ...
    if (result.ok) {
      ...
      SDL_LogCritical(SDL_LOG_CATEGORY_APPLICATION, "Markdown file path: %s", markdown_file_path);
      #ifdef DEBUG
      SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Markdown file path: %s", markdown_file_path);
      #endif
    }
    else {
  ...
  if (!markdown_file_details) {
    #ifdef DEBUG
    SDL_LogCritical(SDL_LOG_CATEGORY_APPLICATION, "Unable to open file: %s", markdown_file_path);
    #endif
    memsafe_boii_api__clean();
    return 1;
  }
  #ifdef DEBUG
  SDL_LogCritical(SDL_LOG_CATEGORY_APPLICATION, "Blob: %s", markdown_file_details->content);
  #endif
  ...
  parser_func_params* params = parser_func_param__new(markdown_file_details->content, sizeof(char)*markdown_file_details->length, parser, NULL);
  if (!params) {
    #ifdef DEBUG
    SDL_LogCritical(SDL_LOG_CATEGORY_APPLICATION, "Unable to allocate memory for params struct.");
    #endif
    memsafe_boii_api__clean();
    return 1;
  }
  ...
  // Creating window
  Uint32 window_flags = SDL_WINDOW_INPUT_FOCUS | SDL_WINDOW_MOUSE_FOCUS | SDL_WINDOW_RESIZABLE | SDL_WINDOW_SHOWN | SDL_WINDOW_ALLOW_HIGHDPI;
  SDL_Window* window = SDL_CreateWindow("Markdown Viewer", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1080, 720, window_flags);
  if (!window) {
    SDL_LogCritical(SDL_LOG_CATEGORY_RENDER, "Unable to create window: %s", SDL_GetError());
  ...
  if (!renderer) {
    SDL_LogCritical(SDL_LOG_CATEGORY_RENDER, "Unable to create renderer: %s", SDL_GetError());
  ...
  HANDLE parsing_thread_handle = CreateThread(NULL, 0, &threaded_parsing, params, 0, NULL);
  if (!parsing_thread_handle) {
    #ifdef DEBUG
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unable to create thread for parsing!");
    #endif
    memsafe_boii_api__clean();
    return 1;
  }
  SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Lauched parsing in a seperate thread.");
  ...
  while (!parsing_complete && !quit) {
    ...
    if (result == WAIT_OBJECT_0) {
      ...
      SDL_Surface* text_surface = TTF_RenderText_Blended(segoeui_font, "* Parsing Complete :)", (SDL_Color){.r=255, .g=255, .b=255, .a=255});
      if (!text_surface) {
        #ifdef DEBUG
      ...
      if (!text_texture) {
        #ifdef DEBUG
        SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Error while creating texture from text surface: %s", SDL_GetError());
        #endif
        continue;
      }

      SDL_Rect destination_rect = {.x=10, .y=10, .w=text_surface->w, .h=text_surface->h};
      ...
      // still parsing
      SDL_Surface* text_surface = TTF_RenderText_Blended(segoeui_font, "Parsing ...", (SDL_Color){.r=255, .g=255, .b=255, .a=255});
      if (!text_surface) {
        #ifdef DEBUG
        SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Error while rendering text surface: %s", TTF_GetError());
        #endif
